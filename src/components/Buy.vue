<template>
  <div>
    ...
    <div style="margin-bottom: 2rem">
      <span>最新价：<span style="font-weight: 700">{{ trade_data.price }}</span></span>
      <span style="margin-left: 1rem">跌停：<span style="font-weight: 700">{{ form.lprice }}</span></span>
      <span style="margin-left: 1rem">涨停：<span style="font-weight: 700">{{ form.hprice }}</span></span>
    </div>
    <!-- 买卖操作 -->
    <a-form-model :layout="form.layout" :model="form">
      ...
      <a-form-item>
        <a-button type="primary" @click="onSubmit" :disabled="is_null"> 买入 </a-button>
      </a-form-item>
    </a-form-model>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onChangeVolume(value) {
      ...
      this.form.max_volume =
        Math.floor(
          Number(
            (this.user_info.restAsset - this.form.volume * this.form.price) /
              10000
          )
        ) * 100;
    },
